...
The next sections apply this model to specific sanitiser chemistries ([Sanitising chemicals](../02-chemicals/01-sanitising.md)) and specific equipment ([Equipment](../03-equipment/01-bucket-fermenter.md)). Each material rating in the [materials register](./02-materials-register.md) traces back to the numbers and zones established here.

---

*This page uses order-of-magnitude estimates for film thickness and evaporation rate derived from thin-film physics. Exact values depend on ambient conditions and surface geometry. The estimates are presented as such and should not be treated as experimentally measured results. The CF=250 derivation and the DDBSA-in-beer calculation are arithmetically exact given the input concentrations; input concentrations are taken from product SDSs cited in the [sources log](../05-references/01-references.md).*
