...
The Foundations section establishes the analytical framework used throughout the rest of the guide. It does not make product-specific recommendations — those are in the Equipment and Chemicals sections — but it provides the concepts and tools that make those recommendations legible.

The sections that follow cover:

- **Materials register** — the plastics, elastomers, and metals commonly found in homebrewing equipment, with compatibility ratings for the chemicals they encounter
- **The wet-dry cycle model** — the mechanism by which sanitiser residue concentrates on drying surfaces, with worked calculations establishing when it matters and when it does not
- **Toxicology** — which materials carry a health risk, what the mechanism is, and how to rank the risks in order of practical concern
- **Microplastics** — what the current evidence says about microplastic content in homebrew beer and how packaging choice affects it
- **The documentation problem** — a structured account of the gap between what equipment sellers claim and what they can actually demonstrate, and why this matters for the brewer

The rest of the guide — Chemicals, Equipment, Processes — builds on these foundations. Each page in those sections cross-references back here for the underlying reasoning.

## A note on scope

This guide focuses on cleaning and sanitisation chemistry and its interaction with equipment materials. It does not attempt to cover all aspects of homebrewing. Fermentation science, recipe design, water chemistry, and yeast management are all outside scope here — John Palmer's *How to Brew* (howtobrew.com) remains the best starting point for those topics, and this guide is in some ways a companion to it rather than a replacement.
...
[^1]: KegLand, *DuoTight Design Revision* (product document). Available at: https://kegland.com.au/cdn/shop/files/duotight_design_revision.pdf — accessed April 2026. Archived copy: [`/static/references/duotight_design_revision.pdf`](/references/duotight_design_revision.pdf).
