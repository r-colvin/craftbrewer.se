...
**Primary observation — brief IPA contact.** One minute of 70% IPA contact (towel application to the stem) produced no visible or tactile change on either of two Enolandia airlocks tested (compact 2-piece GPPS and 6-bubble SBC) — no hazing, no crazing, no change in surface feel. This is consistent with the **B** rating for brief DES contact on GPPS — see [DES compatibility](#compatibility--des-gpps-b-san-a) below — and with Enolandia's manufacturer compatibility claim for ethanol. It does not extend to sustained immersion, and it does not resolve material grade questions — both GPPS and SBC survived brief IPA contact at this timescale.
...
The table below covers the styrene-based articles currently in the register. These are common examples — not an exhaustive list. A reader with an unlisted article should use the table as a reference, applying the principles on this page to their specific article and its geometry. The zone model applies: a new PS or SAN article in an open-surface geometry is Zone A; confined or compressed geometry is Zone B or C.
