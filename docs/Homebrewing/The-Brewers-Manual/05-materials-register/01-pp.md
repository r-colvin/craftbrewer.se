...
The [Witre food grade PP bucket](https://www.witre.se/sv/wsw/livsmedelsgodkand-plasthink-med-lock-1794m415), manufactured by Plast-Box S.A. (Słupsk, Poland), is the best-documented PP article in the homebrewing market.[^1] The <GlossaryTerm term="DoC">DoC</GlossaryTerm> is publicly linked from the Witre product page and covers migration testing against all five EU simulants by an accredited laboratory. Results ranged from less than 0.5 mg/dm² (distilled water, 10% ethanol) to 8.3 mg/dm² (isooctane) — all within the 10 mg/dm² <GlossaryTerm term="OML">OML</GlossaryTerm>. The <GlossaryTerm term="DoC">DoC</GlossaryTerm> covers the bucket as a general-purpose food container; it does not address repeated fermentation use — not because that use is excluded, but because the <GlossaryTerm term="DoC">DoC</GlossaryTerm> was never written with it in mind.

The [Mr-Malt 5 L bucket](https://www.mr-malt.se/) <GlossaryTerm term="DoC">DoC</GlossaryTerm> (available on request from the retailer) is explicit on one point: the article does not comply with the repeated-use provisions of Regulation 10/2011.[^2] We are not aware of any <GlossaryTerm term="DoC">DoC</GlossaryTerm> in the homebrewing market that formally certifies a general-purpose PP bucket for dozens or hundreds of repeated fermentation cycles. This is a documentation gap, not evidence that repeated use is unsafe — migration decreases with each successive contact as the extractable fraction depletes.

The [Bryggbolaget bucket](https://www.bryggbolaget.se/) carries no <GlossaryTerm term="DoC">DoC</GlossaryTerm> but is confirmed PP by the RIC 5 code. For normal homebrewing at ambient fermentation temperatures with standard cleaning and sanitising chemistry, PP's chemical compatibility means you are operating far from any concern even without documentation. The limits that matter most are structural — temperature — rather than chemical.
