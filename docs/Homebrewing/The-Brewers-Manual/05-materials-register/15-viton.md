---
sidebar_position: 15
title: Viton (FKM fluoroelastomer)
---

# Viton (FKM fluoroelastomer)

Viton is DuPont's brand name for fluoroelastomers — polymers with a partially or fully fluorinated backbone providing exceptional resistance to a wide range of aggressive chemicals. Viton appears in homebrewing as spray bottle seals (Biltema pH 1–9 bottle), some O-rings, and some tap seals.

## A critical distinction: cure systems

Viton comes in multiple cure systems with meaningfully different chemical resistance at low pH. The distinction is not commonly stated on product pages, but it determines the rating.

### Viton — peroxide-cured (PP grade)

| | ABNS | DES | PBW | Beer/wort |
|---|---|---|---|---|
| **Rating** | *A–B* | *B* | *A* | *A* |

Peroxide-cured Viton is resistant to phosphoric acid across a wide concentration range and rates *A–B* for ABNS contact. Some minor swelling in DDBSA at high concentrations is manageable in realistic contact durations.

### Viton — polyamine-cured (PA grade)

| | ABNS | DES | PBW | Beer/wort |
|---|---|---|---|---|
| **Rating** | *C* | *B* | *A* | *A* |

Polyamine-cured Viton is rated Not Recommended for phosphoric acid in the Epoca spray bottle compatibility chart — a manufacturer source identifying cure-system-specific incompatibility with the acid component of ABNS.[^1] The polyamine cure system is attacked by phosphoric acid, causing progressive degradation. Rating: *C*.

## Food contact status

Viton fluoroelastomers are used in food contact applications and comply with relevant food contact standards where specified. Cure system should be confirmed in the food contact declaration.

## The identification problem

A component identified as "Viton" without cure system confirmation cannot be reliably assigned a grade for ABNS. Until the cure system is confirmed by the manufacturer, assume polyamine-cured (the more cautious position) and rate accordingly.

The Biltema pH 1–9 spray bottle Viton seal — cure system unconfirmed as of April 2026 — is an open item.

## Compatibility — DES: *B* (both grades)

Viton can swell in high-concentration ethanol. This is a documented limitation despite Viton's general reputation for broad chemical resistance. Rated *B* for DES contact for both cure systems.

## Compatibility — PBW / beer/wort: *A* (both grades)

No concern for either cure system.

---

[^1]: Epoca, *Spray Bottle Chemical Compatibility Chart* (product reference document) — Viton-PA rated Not Recommended for phosphoric acid; Viton-PP rated Recommended (accessed April 2026)
